...
?>
  ...
  <script>
        ...
        const firebaseConfig = {
          apiKey: "AIzaSyADnKxnJ166dEKSGkwL0BFuA6Fm3ZeDXMY",
          authDomain: "mychat2022-3fe4a.firebaseapp.com",
          databaseURL: "https://mychat2022-3fe4a-default-rtdb.asia-southeast1.firebasedatabase.app",
          projectId: "mychat2022-3fe4a",
          storageBucket: "mychat2022-3fe4a.appspot.com",
          messagingSenderId: "1029276949558",
          appId: "1:1029276949558:web:43919e29af86f128becc97"
        };
...
//#1. 메시지 보내기
//#1-1. 전송버튼 클릭시 메시지 보내기 진행
function sendMessage(){
  const message = document.querySelector("#message").value;
  console.log(message);
  if(message.trim() == ""){
    console.log("메시지 없음");
    alert("메시지 입력란에 메시지를 입력해 주세요.");
  }else{
...
//#1-2. 입력상자에서 입력 후 엔터 키보드 이벤트를 통해, 메시지 보내기 진행
...
//#2. 메시지 리스트 가져오기 (내가 보낸 메시지와 상대방들이 보낸 메시지에 대한 구분)
firebase.database().ref("messages").on("child_added", function(snapshot){
  ...
  console.log(snapshot.val().message);  //전송 메시지

  if(snapshot.val().senderId == userId){  //내가 작성한 메시지
    let html = `
  ...
  //신규 입력 메시지가 리스트 공간의 하단에 위치함과 동시에 스크롤바를 하단까지 내리는 역할 
